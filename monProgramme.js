...
fs.readFile(fileName, "utf8", (error, data) => {
  ...
  console.log(`Bubble Sort : ${count.value} comparaisons [${sortBubble}]`);
  const sCount = { value: 0 };
  let sortInsert = insertSort(array, array.length, sCount);
  console.log(`Insert Sort : ${sCount.value} comparaisons [${sortInsert}]`);
  const tCount = { value: 0 };
  let sortSelect = selectSort(array, array.length, tCount);
  console.log(`Select Sort : ${tCount.value} comparaisons [${sortSelect}]`);
  const qCount = { value: 0 };
  let quickSelect = quickSort(array, array.length, qCount);
  console.log(`Quick Sort : ${qCount.value} comparaisons [${quickSelect}]`);
});
